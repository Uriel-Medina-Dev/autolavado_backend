...
import crud.crud_vehicles_services as crud  # 👈 IMPORTANTE: esto es 'crud', no 'crud_completo'
...
def delete_vs(
    ...
    db_item = crud.delete_vehicle_service(db=db, vs_id=vs_id, hard_delete=hard_delete)
    if not db_item:
        raise HTTPException(status_code=404, detail="Item not found")
...
def get_asignaciones_completas(
    ...
    """
    Obtener asignaciones con información completa mediante JOINs
    """
    # 👈 CORREGIDO: usar 'crud' en lugar de 'crud_completo'
    return crud.get_asignaciones_completas(
        db=db,
        skip=skip,
        limit=limit,
...
def get_asignacion_completa(
    ...
    # 👈 CORREGIDO: usar 'crud' en lugar de 'crud_completo'
    result = crud.get_asignacion_completa_by_id(db=db, asignacion_id=asignacion_id)
    if not result:
        raise HTTPException(status_code=404, detail="Asignación no encontrada")
    return result


@router.get("/reportes/resumen-ventas")
def get_resumen_ventas(
    ...
    # 👈 CORREGIDO: usar 'crud' en lugar de 'crud_completo'
    return crud.get_resumen_ventas(
        db=db,
        fecha_inicio=fecha_inicio,
        fecha_fin=fecha_fin
    )
...
def get_ventas_por_dia(
    ...
    # 👈 CORREGIDO: usar 'crud' en lugar de 'crud_completo'
    return crud.get_ventas_por_dia(db=db, mes=mes, año=año)
...
def get_estadisticas_cajero(
    ...
    # 👈 CORREGIDO: usar 'crud' en lugar de 'crud_completo'
    return crud.get_estadisticas_cajero(
        db=db,
        cajero_id=cajero_id,
        fecha_inicio=fecha_inicio,
        fecha_fin=fecha_fin
